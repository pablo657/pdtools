...
curl -s -k -u %2":"%3 --data-binary @flushCacheREQ.xml https://%hostDP%/service/mgmt/3.0 > pingRealizado.xml
for /F "delims=" %%a in ('findstr /m /r "OK" flushCacheRES.xml') do set "salidaCache=%%a"

if "%salidaCache%" == "flushCacheRES.xml" (
	Echo -- Limpieza Cache Exitoso.
	set salidaCache=enproceso
) else (
	Echo 400
	pause
)
