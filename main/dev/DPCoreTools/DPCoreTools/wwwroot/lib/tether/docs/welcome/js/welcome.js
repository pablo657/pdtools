(function() {
  ...
  setupHero = function() {
    ...
    finalDropState = function() {
      a(drops['top left'].dropContent).html('Marrying DOM elements for life.');
      a(drops['bottom right'].dropContent).html('<a class="button" href="http://github.com/HubSpot/tether">★ On Github</a>');
      drops['top left'].open();
  ...
  setupBrowserDemo = function() {
    ...
    $browserDemo = a('.browser-demo.showcase');
    $startPoint = a('.browser-demo-start-point');
    $stopPoint = a('.browser-demo-stop-point');
    $iframe = a('.browser-window iframe');
    $browserContents = a('.browser-content .browser-demo-inner');
    $sections = a('.browser-demo-section');
    ...
    return setSection = function(section) {
      ...
      a('.section-copy').removeClass('active');
      a(".section-copy[data-section=\"" + section + "\"]").addClass('active');
      ...
      closeAllItems = function() {
        return $iframe.contents().find('.item').each(function() {
          return a(this).data().drop.close() || true;
        });
      };
